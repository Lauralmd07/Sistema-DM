...
async def migrate_legacy(collection, doc):
    ...
    try:
        ...
        grid_in = await files_bucket.open_upload_stream(doc.get("filename") or "arquivo", metadata={"document_id": doc["id"], "owner_id": doc.get("owner_id"), "legacy_migration": True})
        for start in range(0, len(content), CHUNK_SIZE):
            await grid_in.write(content[start:start + CHUNK_SIZE])
        await grid_in.close()
        await collection.update_one({"id": doc["id"]}, {"$set": {"gridfs_id": str(grid_in._id)}, "$unset": {"file_data": ""}})
...
async def upload_client_document_gridfs(client_id: str, file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    ...
    gridfs_id, file_size = await save_upload(file, {"document_id": document_id, "owner_id": current_user["id"], "client_id": client_id, "kind": "client_document"})
    doc = {"id": document_id, "client_id": client_id, "owner_id": current_user["id"], "filename": file.filename or "arquivo", "file_type": file.content_type or "application/octet-stream", "file_size": file_size, "gridfs_id": gridfs_id, "created_at": datetime.now(timezone.utc)}
    try:
...
async def upload_document_gridfs(folder_id: str = Form(...), file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    if not await db.folders.find_one({"id": folder_id, "owner_id": current_user["id"]}):
        raise HTTPException(status_code=404, detail="Pasta não encontrada")
    document_id = server.uuid.uuid4().__str__()
    gridfs_id, file_size = await save_upload(file, {"document_id": document_id, "owner_id": current_user["id"], "folder_id": folder_id, "kind": "drive_document"})
    doc = {"id": document_id, "folder_id": folder_id, "owner_id": current_user["id"], "filename": file.filename or "arquivo", "file_type": file.content_type or "application/octet-stream", "file_size": file_size, "gridfs_id": gridfs_id, "created_at": datetime.now(timezone.utc)}
    try:
