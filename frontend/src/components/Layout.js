    ...
    <div className="min-h-screen bg-[#121212] text-[#F5F5F5]">
      <aside className={`fixed top-0 left-0 h-full bg-[#1E1E1E] border-r border-[#3A3A3A] transition-all duration-300 z-30 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all ${isActive ? 'bg-[#D4AF37] text-[#121212]' : 'hover:bg-[#2A2A2A] text-[#F5F5F5]'}`}
              >
                <Icon size={20} />
        ...
        <div className="p-4 border-t border-[#3A3A3A]">
          <button onClick={handleLogout} data-testid="logout-btn" className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-red-900/20 text-red-400">
            <LogOut size={20} />
