...
import { Calendar, FolderKanban, DollarSign, FolderTree, LogOut, Menu, X, User, Users, Gavel } from 'lucide-react';
...
export const Layout = ({ children }) => {
  ...
  const menuItems = [
    ...
    { path: '/prazos-audiencias', icon: Gavel, label: 'Prazos e Audiências' },
    { path: '/processos', icon: FolderKanban, label: 'Processos' },
    { path: '/clientes', icon: Users, label: 'Clientes' },
    { path: '/drive', icon: FolderTree, label: 'Drive Jurídico' },
    ...(user?.role === 'admin' ? [{ path: '/financeiro', icon: DollarSign, label: 'Financeiro' }] : []),
  ];
    ...
    <div className="min-h-screen bg-[#121212] text-[#F5F5F5]">
      <aside className={`fixed top-0 left-0 h-full bg-[#1E1E1E] border-r border-[#3A3A3A] transition-all duration-300 z-30 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-[#3A3A3A]">
          ...
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-[#2A2A2A] rounded-lg" data-testid="sidebar-toggle-btn" aria-label={sidebarOpen ? 'Recolher menu' : 'Expandir menu'}>
        ...
        <div className="p-4 border-b border-[#3A3A3A]">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-[#D4AF37] rounded-full flex items-center justify-center">
              <span className="text-[#121212] font-bold">{user?.name?.charAt(0).toUpperCase()}</span>
            </div>
            {sidebarOpen && <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{user?.name}</p><p className="text-xs text-gray-400 capitalize">{user?.role}</p></div>}
          </div>
        </div>
